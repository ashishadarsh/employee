...
const httpLink = concat(authLink, createHttpLink({
    uri: environment.graphqlHttpUrl || 'http://localhost:9000/graphql',
  })
);

const wsLink = new GraphQLWsLink(createWsClient({
  url: environment.graphqlWsUrl || 'ws://localhost:9000/graphql',
}))
...
export async function createNewTask({_id, title, description, empId, completionDate, status, type, priority, pinned, backlog, tags}) {
  const mutation = gql`
